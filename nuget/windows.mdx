...
IPSA’s NuGet package is a thin .NET wrapper over the same native Rust engine used by the desktop app.
...
The package does **not** reimplement annotation logic in C#; it forwards your inputs to the native engine for maximum speed and parity with IPSA.

## Data flow

```text
Spectrum (mz/intensity) + metadata
          |
          v
Ipsa.Native (C# P/Invoke)
          |
          v
ipsa_ffi.dll (Rust FFI -> IPSA core)
          |
          v
AnnotationResponse JSON -> typed C# objects
```
